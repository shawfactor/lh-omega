...
function lh_omega_theme_setup() {
	...
	add_action( 'omega_before_entry', 'lh_omega_entry_header' );
	remove_action( 'omega_before_loop', 'omega_archive_title'); 

}

add_action( 'after_setup_theme', 'lh_omega_theme_setup', 11 );
 ...
 */
function lh_omega_entry_header() {
...
if ( ! function_exists( 'lh_omega_return_fonts_url' ) ) :
/**
 ...
 */
function lh_omega_return_fonts_url() {
	$fonts_url = '';
	$fonts     = array();
...
function delicious_scripts_styles() {
	$query_args = array(
 	...
 	wp_enqueue_style('google-fonts', esc_url( lh_omega_return_fonts_url()), array(), null  );
